...
export async function chromeConnect(args) {
    ...
    try {
        ...
        return errorResponse(`Error connecting to Chrome: ${error}\n\nMake sure Chrome is running with:\nchrome --remote-debugging-port=${port}`);
    }
}
/**
...
export async function chromeLaunch(args) {
    ...
    try {
        ...
        const result = await browserManager.launch(debugPort, connectionId, headless, userDataDir, extraArgs);
...
export async function connect(args) {
    ...
    if (existingPage) {
        ...
        try {
            ...
            return errorResponse(`Failed to navigate existing connection: ${error}\n\n` +
    ...
    if (args.port !== undefined) {
        ...
        if (!portInUse) {
            return errorResponse(`Nothing is running on port ${port}.\n\n` +
        ...
        try {
            ...
            return errorResponse(`Failed to connect to Chrome at ${host}:${port}: ${error}\n\n` +
                `${suggestion}\n\n` +
    ...
    while (attempts < maxAttempts) {
        ...
        console.error(`[cherry-chrome] Port ${randomPort} is in use, trying another random port (attempt ${attempts + 1}/${maxAttempts})...`);
        randomPort = getRandomPort();
        attempts++;
    }
    if (attempts >= maxAttempts) {
        return errorResponse(`Failed to find an available port after ${maxAttempts} attempts.\n\n` +
    ...
    try {
        const launchResult = await browserManager.launch(randomPort, connectionId, headless, args.user_data_dir, args.extra_args);
...
export async function chromeListConnections() {
    ...
    if (connections.size > 1) {
        lines.push('Use chrome_switch_connection(connection_id) to change the active connection.');
    }
...
export async function listTargets(args) {
    try {
        ...
        if (!connection) {
            ...
            return errorResponse(`No Chrome connection '${id}' found. Use chrome_connect() or chrome_launch() first.`);
        }
...
export async function switchTarget(args) {
    try {
        ...
        if (target.type !== 'page') {
            return errorResponse(`Target "${target.title}" is a ${target.type}, not a page. Can only switch to page targets.`);
        }
...
export async function enableDebugTools(args) {
    try {
        ...
        return successResponse(`Debug tools enabled successfully
