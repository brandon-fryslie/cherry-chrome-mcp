 ...
 */
function findSimilarSelectors(attemptedSelector, inventory) {
    const suggestions = [];
 ...
 */
export async function gatherZeroResultSuggestions(page, attemptedSelector) {
    const lines = [];
    try {
    ...
    `;
        const inventory = await page.evaluate(script);
 ...
 */
export async function gatherActionContext(page, selector, action, beforeSnapshot, connectionId) {
    const lines = [''];
    try {
 ...
 */
export function computeDOMDiff(before, after) {
    const diff = {
        hasChanges: false,
        countChanges: {},
        added: [],
    ...
    for (const key of beforeKeys) {
        if (afterKeys.has(key)) {
            ...
            const changes = [];
            if (b.text !== a.text)
                changes.push(`text "${b.text}" → "${a.text}"`);
            if (b.visible !== a.visible)
                changes.push(`visible ${b.visible} → ${a.visible}`);
            if (b.disabled !== a.disabled)
                changes.push(`disabled ${b.disabled} → ${a.disabled}`);
            if (b.value !== a.value)
                changes.push(`value "${b.value}" → "${a.value}"`);
            if (changes.length > 0) {
                diff.hasChanges = true;
 ...
 */
export function formatDOMDiff(diff) {
    if (!diff.hasChanges) {
        return '--- No DOM changes detected ---';
    }
    const lines = ['--- DOM Changes ---'];
