 ...
 */
function formatLogWithStack(log, expandErrors) {
    const lines = [];
...
export async function getConsoleLogs(args) {
    ...
    try {
        ...
        if (expandErrors) {
            ...
            output.push(`Showing ${recentLogs.length} of ${filteredLogs.length}${filterLevel !== 'all' ? ` (filter: ${filterLevel})` : ''} (with stack traces):`);
            output.push('');
            ...
            output.push(`Showing ${showing} of ${totalCompressedItems} compressed entries (${filteredLogs.length} raw messages)${filterLevel !== 'all' ? ` (filter: ${filterLevel})` : ''}:`);
