...
function toPuppeteerKey(mainKey) {
    if (mainKey === 'Space')
        return ' ';
    if (mainKey.length === 1) {
        const c = mainKey.charCodeAt(0);
        if (c >= 48 && c <= 57)
            return `Digit${mainKey}`; // '0'-'9'
        if (c >= 97 && c <= 122)
            return `Key${mainKey.toUpperCase()}`; // 'a'-'z'
...
export async function pressKey(args) {
    ...
    try {
        ...
        // intended `key` value.
        const keyToPress = toPuppeteerKey(mainKey);
