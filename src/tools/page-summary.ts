 ...
 */
const DEFAULT_CONFIG: Required<PageSummaryConfig> = {
  include: {
    focused: true,
    buttons: true,
    links: true,
...
export async function gatherPageSummary(
  ...
  if (cfg.include.buttons) {
    ...
    if (buttons.total > 0) {
      const lines: string[] = [`── Buttons (${buttons.total}) ──`];
  ...
  if (cfg.include.inputs) {
    ...
    if (inputs.total > 0) {
      const lines: string[] = [`── Inputs (${inputs.total}) ──`];
  ...
  if (cfg.include.links) {
    ...
    if (links.total > 0) {
      const lines: string[] = [`── Links (${links.total}) ──`];
  ...
  if (cfg.include.forms) {
    ...
    if (forms.total > 0) {
      const lines: string[] = [`── Forms (${forms.total}) ──`];
  ...
  if (cfg.include.toggles) {
    ...
    if (toggles.total > 0) {
      const lines: string[] = [`── Toggles (${toggles.total}) ──`];
  ...
  if (cfg.include.headings) {
    ...
    if (headings.total > 0) {
      const lines: string[] = [`── Headings (${headings.total}) ──`];
