 ...
 */
export async function inspectElement(args: InspectElementArgs): Promise<ToolResult> {
  try {
    ...
    if (args.description) queryParts.push(`"${args.description}"`);
    if (args.text_contains) queryParts.push(`text: "${args.text_contains}"`);
    if (args.tag) queryParts.push(`tag: ${args.tag}`);
    if (args.attributes) {
      const attrParts = Object.entries(args.attributes)
        .filter(([_, v]) => v !== undefined)
        .map(([k, v]) => `${k}="${v}"`);
      if (attrParts.length > 0) queryParts.push(attrParts.join(', '));
    }
    ...
    const rawResult = await page.evaluate(script) as {
      candidates: Array<{
        element: string; // serialized element data
        selector: string;
        stability: number;
        strategy: string;
        count: number;
    ...
    // Parse candidates
    const candidates: SelectorCandidate[] = rawResult.candidates.map(c => ({
      selector: c.selector,
 ...
 */
function buildDiscoveryScript(args: InspectElementArgs): string {
 ...
 */
function formatInspectResult(result: InspectElementResult): string {
  const lines: string[] = [];
