...
import type { ToolHandler } from '../src/toolRegistry.js';
...
describe('Tool Handler Creation', () => {
  ...
  describe('Handler Name Validation', () => {
    it('should verify handler names match tool definition names (legacy)', () => {
      ...
      assert.strictEqual(
        uniqueNames.size,
        allLegacyTools.length,
        'Legacy tools should have no duplicate names'
      );
    ...
    it('should verify handler names match tool definition names (smart)', () => {
      ...
      assert.strictEqual(
        uniqueNames.size,
        allSmartTools.length,
        'Smart tools should have no duplicate names'
      );
    ...
    it('should verify no overlap between legacy-specific and smart-specific tools', () => {
      ...
      assert.strictEqual(
        overlap.length,
        0,
        `Legacy and smart specific tools should not overlap, found: ${overlap.join(', ')}`
      );
    });
    ...
    it('should verify shared tools are not in specific tool lists', () => {
      ...
      assert.strictEqual(
        legacyOverlap.length,
        0,
        `Legacy-specific tools should not include shared tools, found: ${legacyOverlap.join(', ')}`
      );
      ...
      assert.strictEqual(
        smartOverlap.length,
        0,
        `Smart-specific tools should not include shared tools, found: ${smartOverlap.join(', ')}`
      );
    });
  });
  ...
  describe('Mode-Specific Tool Presence', () => {
    it('should verify chrome_connect only exists in legacy mode', () => {
      assert.ok(
        LEGACY_SPECIFIC_TOOL_NAMES.includes('chrome_connect'),
        'chrome_connect should be in legacy-specific tools'
      );
      assert.ok(
        !SMART_SPECIFIC_TOOL_NAMES.includes('chrome_connect'),
        'chrome_connect should not be in smart-specific tools'
      );
    });

    it('should verify connect only exists in smart mode', () => {
      assert.ok(
        SMART_SPECIFIC_TOOL_NAMES.includes('connect'),
        'connect should be in smart-specific tools'
      );
      assert.ok(
        !LEGACY_SPECIFIC_TOOL_NAMES.includes('connect'),
        'connect should not be in legacy-specific tools'
      );
    });

    it('should verify debugger_enable only exists in legacy mode', () => {
      assert.ok(
        LEGACY_SPECIFIC_TOOL_NAMES.includes('debugger_enable'),
        'debugger_enable should be in legacy-specific tools'
      );
      assert.ok(
        !SMART_SPECIFIC_TOOL_NAMES.includes('debugger_enable'),
        'debugger_enable should not be in smart-specific tools'
      );
    });

    it('should verify enable_debug_tools only exists in smart mode', () => {
      assert.ok(
        SMART_SPECIFIC_TOOL_NAMES.includes('enable_debug_tools'),
        'enable_debug_tools should be in smart-specific tools'
      );
      assert.ok(
        !LEGACY_SPECIFIC_TOOL_NAMES.includes('enable_debug_tools'),
        'enable_debug_tools should not be in legacy-specific tools'
      );
    });
  });
});
