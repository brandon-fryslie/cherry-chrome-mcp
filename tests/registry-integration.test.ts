...
describe('Registry Integration', () => {
  ...
  const createMockHandler = (
    name: string,
    definition: Tool,
    behavior: (args: unknown) => Promise<ToolResult>
  ): ToolHandler => ({
    name,
    definition,
    invoke: behavior,
  ...
  describe('successful tool execution', () => {
    it('should return successful response for valid tool request', async () => {
      const handlers = new Map<string, ToolHandler>([
        [
          'mock_tool',
          createMockHandler('mock_tool', mockTools[0], async (args: unknown) => {
    ...
    it('should extract connection_id from args for error context', async () => {
      ...
      const handlers = new Map<string, ToolHandler>([
        [
          'mock_tool',
          createMockHandler('mock_tool', mockTools[0], async (args: unknown) => {
            const typedArgs = args as { connection_id?: string };
    ...
    it('should simulate full request->registry->handler->response flow', async () => {
      ...
      const handlers = new Map<string, ToolHandler>([
        [
          'mock_tool',
          createMockHandler('mock_tool', mockTools[0], async (args: unknown) => {
            const typedArgs = args as { input: string; connection_id: string };
            return {
              content: [
                {
                  type: 'text',
