...
import type { ConsoleMessage, Connection } from '../src/types.js';
import {
  filterLogsByLevel,
  limitLogs,
  ...
  updateQueryTracking,
} from '../src/tools/console-logs.js';

// ============================================================================
// Test Fixtures
// ============================================================================

function createLog(overrides: Partial<ConsoleMessage> = {}): ConsoleMessage {
  return {
    level: 'log',
    text: 'test message',
    timestamp: Date.now(),
...
function createConnectionState(
  ...
  return {
    lastQueryEpoch: null as number | null,
    lastConsoleQuery: null as number | null,
    navigationEpoch: 1,
    lastNavigationTime: Date.now() - 5000,
    hmrUpdateCount: 0,
    lastHmrTime: null as number | null,
    ...overrides,
  };
}
...
describe('formatPageStateHeader', () => {
  it('should format first query state', () => {
    const state = {
      changeStatus: 'first_query' as const,
      navigationEpoch: 1,
      lastNavigationTime: Date.now() - 5000,
      hmrUpdateCount: 0,
      lastHmrTime: null,
    ...
    assert.ok(result.some((line: string) => line.includes('Navigation epoch: 1')));
    // Should NOT include change status for first query
    assert.ok(!result.some((line: string) => line.includes('[PAGE RELOADED')));
  });

  it('should format reloaded state', () => {
    const state = {
      changeStatus: 'reloaded' as const,
      navigationEpoch: 2,
  ...
  it('should include HMR info when present', () => {
    const now = Date.now();
    const state = {
      changeStatus: 'hmr_updated' as const,
      navigationEpoch: 1,
      lastNavigationTime: now - 10000,
      hmrUpdateCount: 3,
    ...
    assert.ok(result.some((line: string) => line.includes('HMR updates since navigation: 3')));
    assert.ok(result.some((line: string) => line.includes('Last HMR update:')));
  });
});
...
describe('formatLogWithStack', () => {
  ...
  it('should include stack trace for errors when expandErrors is true', () => {
    ...
    assert.ok(result.some((line: string) => line.includes('at foo')));
  });
...
describe('formatProcessedLogs', () => {
  it('should show empty message when no logs', () => {
    ...
    assert.ok(result.some((line: string) => line.includes('No console messages captured')));
  });
  ...
  it('should include filter info in empty message', () => {
    ...
    assert.ok(result.some((line: string) => line.includes('(filter: error)')));
  });
});
